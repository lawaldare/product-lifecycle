...
class Producer { //Producer constructor for the Producer state
	constructor(state, mainProduct, link) {
		...
		this.go =  async function() { //method that change the state
			...
			this.state.change(new file.Retailer(this.state, this.mainProduct)); //change the state and it stops the production after the last product passed the recycler.
		};
	}
}
